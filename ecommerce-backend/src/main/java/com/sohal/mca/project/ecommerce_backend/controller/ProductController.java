 ...
 * Version: 1.0
 * Description: This controller handles product-related operations in the e-commerce backend.
 * It provides endpoints for creating, updating, deleting, and retrieving products.
 * The controller uses Swagger annotations for API documentation and includes security annotations...
 */
...
public class ProductController {
    ...
    public ResponseEntity<Page<Product>> getAllProducts(
        ...
        logger.debug("Received request for products with pagination and filters. Page: {}, Size: {}", pageable.getPageNumber(), pageable.getPageSize());
    ...
    public ResponseEntity<List<Product>> getProductsByCategoryName(@Parameter(description = "Name of the category to filter products") @PathVariable String categoryName) {
    ...
    @ApiResponse(responseCode = "200", description = "Successfully retrieved products matching the search criteria",
                 content = @Content(mediaType = "application/json", schema = @Schema(implementation = Product.class)))
    @GetMapping("/search")
